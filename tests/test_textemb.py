...
def test_dfc_appends_back_face():
    ...
    assert out == (
        "Creature — Human Werewolf | At night, transform ~. // "
...
def test_cache_miss_without_model_raises_listing_names(data_root, monkeypatch):
    ...
    with pytest.raises(RuntimeError) as excinfo:
        textemb.embed_names(["Blazing Torrent", "Steel Trinket"], paths.TEXT_EMB_CACHE)
...
def test_cache_keys_use_norm_17lands_aliases(data_root, monkeypatch):
    ...
    textemb._write_cache(paths.TEXT_EMB_CACHE, {names.norm("Bespoke Bō"): vec})
...
def test_encoder_extends_cache_then_serves_without_it(data_root, monkeypatch):
    ...
    first = textemb.embed_names(
        ["Blazing Torrent"], paths.TEXT_EMB_CACHE, texts_by_name=texts
    )
