    ...
    with gzip.open(dest, "wt", newline="") as fh:
        ...
        for row in rows:
            meta = [
                ...
                row["pick_number"],
                row["pick"],
                row.get("games_bucket", 500),
                row.get("wr_bucket", 0.60),
                "arbitrary",
            ]
...
def write_old_draft_csv(dest, rows, era="match_buckets", vocab=None, etag="etag-old-1"):
    ...
    header = (
        meta_cols + [f"pack_card_{n}" for n in vocab] + [f"pool_{n}" for n in vocab]
    )
    dest.parent.mkdir(parents=True, exist_ok=True)
    with gzip.open(dest, "wt", newline="") as fh:
        writer = csv.writer(fh)
...
def write_game_csv(dest, games=None, names=None, prefix_names=None, etag="etag-game-1"):
    ...
    header = GAME_META_COLS + [f"{p}{n}" for p in GAME_PREFIXES for n in per_prefix[p]]
    key = {
        "opening_hand_": "oh",
        "drawn_": "drawn",
        "tutored_": "tutored",
...
def hand_game_rows():
    ...
    return [
        ...
        dict(won=True, main_colors="WU", oh={B: 1}, drawn={A: 1}, deck={A: 1, B: 1}),
        dict(
            won=False,
            main_colors="WU",
            oh={A: 1, B: 1},
...
def ratings_rows():
    ...
    rows = [
        dict(
            ...
            opening_hand_win_rate=wr - 0.01,
            drawn_win_rate=wr - 0.02,
...
def write_ratings_cache(
    set_code=SET, limited_type=FMT, date_str="2026-01-01", rows=None
):
...
OPPO_TURN_SUFFIXES = [
    s for s in USER_TURN_SUFFIXES if s not in ("cards_drawn", "cards_tutored")
]
...
def _replay_zero_fill(suffix):
    ...
    if (
        suffix.endswith("_life")
        or suffix.endswith("mana_spent")
...
def write_replay_csv(
    dest, games=None, names=None, etag="etag-replay-1", turn_cols=REPLAY_TURN_COLS
):
    ...
    with gzip.open(dest, "wt", newline="") as fh:
        ...
        for i, game in enumerate(games):
            ...
            row = [
                ...
                "2024-09-24 12:00:00",
                0,
                game.get("match_number", 1),
                1,
                "2024-09-24 19:00:00",
                game.get("rank", "gold"),
                "",
                game.get("main_colors", "WU"),
                "",
                game["on_play"],
                game["num_mulligans"],
                game.get("opp_num_mulligans", 0),
                "UB",
                game["num_turns"],
                game["won"],
            ]
            row += [_replay_cell(cands[k]) if k < len(cands) else "" for k in range(7)]
...
def write_mull_cards_csv():
    ...
    with open(paths.CARDS_CSV, "w", newline="") as fh:
        ...
        for card_id, name in rows:
            writer.writerow([card_id, SET, name, "common", "C", 1.0, "Creature", True])
...
def write_cardfeats():
    ...
    for name, (mana_value, pips, colors, card_type) in CARDFEAT_SPEC.items():
        ...
        rows.append(
            {"name_display": name, "name_norm": names.norm_17lands(name), **row}
        )
    ...
    for i in range(n):
        ...
        games.append(
            dict(
                ...
                won=won,
                num_mulligans=i % 3,
                opp_num_mulligans=(i + 1) % 3,
                num_turns=num_turns,
                deck={CARD_A: 17, CARD_B: 12, CARD_C: 11},
                turns=turns,
            )
        )
    return games
...
class StubOrtSession:
    ...
    def run(self, outputs, feeds):
        ...
        StubOrtSession.last_scorer_feeds = {k: np.array(v) for k, v in feeds.items()}
        logits = feeds["pack_emb"].sum(axis=-1).astype(np.float32)
        logits[feeds["pack_mask"]] = float("-inf")
        return [logits]


def write_foundation_version(
    tag="v1",
    fmt=None,
    wr_id=33,
    games_id=6,
    ...
    np.savez(
        out / "constants.npz",
        pool_null_input=np.full(DRAFTFM_D, DRAFTFM_NULL, dtype=np.float32),
    )
    meta = {
...
def write_draftfm_assets(
    set_code=SET, manifest_hash=FOUNDATION_MANIFEST_HASH, picks_per_pack=14
):
