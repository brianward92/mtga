...
def test_download_corpus_dry_run_expands_registry(download_script, monkeypatch, capsys):
...
def test_download_corpus_forces_no_ratings(download_script, monkeypatch):
    calls = []
    monkeypatch.setattr(
        download_script.download, "sync_cards_csv", lambda force=False: "SKIPPED"
    )
    monkeypatch.setattr(
        download_script.download, "sync_abilities_csv", lambda force=False: "SKIPPED"
    )
    monkeypatch.setattr(
