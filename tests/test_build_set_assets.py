...
from _synth import ALIASES_A, CARD_A, CARD_B, CARD_C, CARD_D, FMT, GRP, SET, VOCAB
...
def curated_vocab(data_root):
    ...
    vocab_file.write_text(json.dumps({"set": SET, "format": FMT, "names": VOCAB}))
    return vocab_file


def test_universe_merges_vocab_store_and_aliases(
    card_store, curated_vocab, ratings_cache
):
...
def test_build_writes_loadable_assets(card_store, curated_vocab, ratings_cache):
    ...
    assert assets["features"][0, featurize.N_FEATURES] == pytest.approx(0.25, rel=1e-2)
...
def test_missing_text_embeddings_fail_unless_allowed(
    card_store, curated_vocab, monkeypatch
):
