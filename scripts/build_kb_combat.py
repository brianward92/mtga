...
COMBAT_KEYWORDS = [
    "Flying", "Reach", "Menace", "Trample", "Deathtouch",
    "First strike", "Double strike", "Lifelink", "Vigilance", "Ward",
    "Hexproof", "Indestructible", "Protection", "Defender", "Flash",
]
...
REMOVAL = [
    ...
    ("damage", re.compile(r"deals? (\d+|X) damage to target (creature|any target)", re.I)),
    ("shrink", re.compile(r"gets? -\d+/-\d+", re.I)),
    ("exile", re.compile(r"exile target (creature|permanent)", re.I)),
    ("bounce", re.compile(r"return target (creature|permanent).{0,40}to (its|their) owner's hand", re.I)),
...
def main():
    ...
    for kw in COMBAT_KEYWORDS:
        ...
        for c in cards:
            text = (c["oracle"] or "")
            if kw in (c.get("keywords") or []) or re.search(rf"\b{re.escape(kw)}\b", text):
                by_colour[colour_of(c)] += 1
        if not by_colour:
            continue
        total = sum(by_colour.values())
        rows.append([kw, total] + [by_colour.get(x, 0) for x in "WUBRG"] + [by_colour.get("C", 0)])
    parts += [table(rows, ["Keyword", "All", "W", "U", "B", "R", "G", "C"]), ""]

    # --- Creature bodies by cost -----------------------------------------
    parts += ["## What a creature costs", "",
    ...
    parts += ["## What open mana threatens", "",
    ...
    parts += ["## Every instant-speed card, by colour", "",
    ...
    for colour in ["W", "U", "B", "R", "G", "C"]:
        group = sorted(by_colour.get(colour, []), key=lambda c: (c["mv"] or 0, c["name"]))
        if not group:
            continue
        parts += [f"### {colour}", ""]
        rows = []
    ...
    for kind, pattern in REMOVAL:
        ...
        rows.append([kind, len(hits), fast,
