...
def load_model(run_dir):
    checkpoint = torch.load(Path(run_dir) / "best.pt", map_location="cpu",
                            weights_only=False)
    ...
    for set_code in sets:
        for fmt in formats:
            frame = predict.foundation_predictions(
                model, set_code, fmt, device=args.device,
                condition_wr_id=args.wr_id, condition_games_id=args.games_id,
            )
            ...
            print(f"{set_code} {fmt}: expert top1 {e['top1']:.4f} "
                  f"(CI {e['top1_ci'][0]:.4f}-{e['top1_ci'][1]:.4f}) "
                  f"top3 {e['top3']:.4f} n={e['n_picks']:,}", flush=True)

    dev_mean = sum(
        s["expert"]["top1"] for k, s in summaries.items()
        if k.endswith("PremierDraft")
    ) / max(sum(1 for k in summaries if k.endswith("PremierDraft")), 1)
    result = {"run": str(args.run), "best_step": checkpoint.get("step"),
              "wr_id": args.wr_id, "dev_mean_expert_top1": dev_mean,
              "summaries": summaries}
