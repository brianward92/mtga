...
def load_expert_pair(run_dir, set_code, fmt, version, split):
    ...
    ceiling = evalproto.validate(
        predict.per_set_model_predictions(set_code, fmt, version=version, split=split)
    )
...
def set_report(set_code, zs_expert, ceil_expert):
    ...
    return {
        ...
        "retention_minus_published_overall_ratio": (
            ...
            if set_code in PUBLISHED_OVERALL_RATIO
            else float("nan")
        ),
    }
...
def main():
    ...
    for set_code in DEV_SETS:
        ...
        print(
            f"== {set_code} (expert slice, F-dev zero-shot vs. per-set "
            f"ceiling {args.version}/{args.split}) =="
        )
        print(
            f"  late (pick>={FIRST_LATE_PICK}) zero-shot top1: "
    ...
    print(
        f"dev-trio mean late-draft retention: {dev_mean_retention:.4f} "
        f"({100 * dev_mean_retention:.1f}%)"
    )
    print(
        f"dev-trio mean overall ratio (this run): {dev_mean_overall:.4f} "
    ...
    print(
        all_curves.sort_values(["set", "pack_number", "pick_number"]).to_string(
            index=False
        )
    )
    ...
    out_path = (
        Path(args.out)
        if args.out
        else (Path(args.run) / "zeroshot" / "late_draft_retention.json")
    )
