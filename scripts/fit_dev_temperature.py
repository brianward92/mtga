...
def create_parser():
    ...
    parser.add_argument(
        "--out", default=None, help="default <run-dir>/temperature_fit/summary.json"
    )
    parser.add_argument(
        "--force", action="store_true", help="recompute logits cache even if present"
    )
    return parser
...
def _load_sessions(model_dir):
    ...
    card = onnxruntime.InferenceSession(
        str(model_dir / "card_encoder.onnx"), providers=providers
    )
    set_enc = onnxruntime.InferenceSession(
        str(model_dir / "set_encoder.onnx"), providers=providers
    )
    scorer = onnxruntime.InferenceSession(
        str(model_dir / "scorer.onnx"), providers=providers
    )
...
def _cross_check_meta(model_dir, run_dir):
    ...
    if not record_path.exists():
        print(
            f"WARNING: no record.json at {record_path}; cannot cross-check "
            f"the ONNX export against the training run."
        )
        return
    record = json.loads(record_path.read_text())
    ok = meta.get("checkpoint_step") == record.get("best_step") and meta.get(
        "checkpoint_val_top1"
    ) == record.get("best_val_top1")
    status = "OK" if ok else "MISMATCH"
    print(
        f"meta.json vs record.json cross-check: {status} "
...
def compute_expert_logits(
    set_code, fmt, sessions, pool_null_input, wr_id, games_id, batch_size
):
    ...
    expert_mask = (
        meta["user_game_win_rate_bucket"].to_numpy() >= evalproto.EXPERT_WR_BUCKET
    ...
    for start in range(0, m, batch_size):
        ...
        position = (
            position_features(torch.from_numpy(context_chunk), ppp)
            .numpy()
            .astype(np.float32)
        )
        batch = {
        ...
        if (start // batch_size) % 20 == 0:
            ...
            print(
                f"  {set_code}: {start + len(chunk):,}/{m:,} " f"({rate:,.0f} picks/s)",
                flush=True,
            )
    ...
    return {
        ...
        "wr_bucket": meta["user_game_win_rate_bucket"]
        .to_numpy()[rows]
        .astype(np.float64),
...
def main():
    ...
    cache_dir = (
        Path(args.cache_dir)
        if args.cache_dir
        else run_dir / "temperature_fit" / "logits_cache"
    )
    out_path = (
        Path(args.out) if args.out else run_dir / "temperature_fit" / "summary.json"
    )
    ...
    if not best_pt.exists():
        print(
            f"NOTE: {best_pt} not found on this machine; using the "
    ...
    base_ranks = {
        s: _base_rank(caches[s]["logits"], caches[s]["pick_pos"].astype(np.int64))
        for s in sets
    }
    ...
    for set_code in sets:
        ...
        print(
            f"{set_code}: ECE(T=1)={ece1:.5f} (published {published}) [{flag}] "
            f"top1={top1_1:.5f}"
        )
    ...
    for temperature in temps:
        ...
        for set_code in sets:
            frame = frame_at_temperature(
                caches[set_code], temperature, base_ranks[set_code]
            )
    ...
    print(
        f"T minimizing mean dev-trio log-loss: T={t_nll:.3f}  "
    ...
    print(
        f"{'set':6s} {'ECE(T=1)':>10s} {'ECE(T*)':>10s} {'logloss(T=1)':>13s} "
        f"{'logloss(T*)':>12s}"
    )
    for set_code in sets:
        print(
