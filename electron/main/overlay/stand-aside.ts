 ...
 */
import { arenaContentBox, sidebarShellFrame } from '../../shared/layout'
...
export function inChromeBand(local: Point, rect: Pick<Rect, 'width' | 'height'>): boolean {
  ...
  const sidebar = sidebarShellFrame(rect as { width: number; height: number })
  if (sidebar.width > 0 && local.x >= sidebar.x && local.y >= sidebar.y) return false
...
export class StandAside {
   ...
   */
  noteClick(local: Point, rect: Pick<Rect, 'width' | 'height'>, now: number): boolean {
    if (now < this.suppressUntil) return this.settle(false)
    return this.settle(inChromeBand(local, rect))
  }
