...
import { DraftSession, type DraftSessionSnapshot } from './draft-session'
...
export class DraftParser extends EventEmitter {
  ...
  /** Return the current session snapshot, or null before a draft starts. */
  getSnapshot(): DraftSessionSnapshot | null {
    return this.session?.snapshot() ?? null
  }

  /** Consume one raw Player.log line without throwing on malformed input. */
