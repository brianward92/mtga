...
export function predictPopout(card: Rect, view: { width: number; height: number }): Rect[] {
  const w = card.width * POPOUT_SCALE
  const h = card.height * POPOUT_SCALE
  const gap = card.width * POPOUT_GAP
  let x = card.x + card.width + gap
  if (x + w > view.width) x = card.x - gap - w
  let y = card.y + card.height / 2 - h / 2
  y = Math.max(0, Math.min(view.height - h, y))
  const preview = { x, y, width: w, height: h }
  // Flavour/quote box: sits between the card's left edge and the preview,
  // level with the preview's top.
