...
def main():
    ...
    def points(names):
        ...
        for name in names:
            ...
            if r and r["train_picks"] and r["dev_mean_top1"] is not None:
                out.append((r["train_picks"] / 1e6,
                            100 * r["dev_mean_top1"], name))
        return out
    ...
    styles = {"bertram": (0, (5, 3)), "expert_tuned": (0, (2, 2)),
              "gpt4o": (0, (1, 2))}
    # Label placement avoids collisions between close anchors (44.5 vs 43).
    placement = {"gpt4o": {"x": 0.0, "ha": "left", "va": "top",
                           "offset": (4, -3)}}
    for key, spec in anchors.items():
        y = 100 * spec["top1"]
        ax.axhline(y, color=REFERENCE, lw=1.0,
                   linestyle=styles.get(key, "--"), zorder=1)
        place = placement.get(key, {"x": 1.0, "ha": "right", "va": "bottom",
                                    "offset": (-4, 3)})
        ax.annotate(spec["label"], xy=(place["x"], y),
                    xycoords=("axes fraction", "data"),
                    xytext=place["offset"], textcoords="offset points",
    ...
    if ladder:
        ...
        ax.plot(xs, ys, "-o", color=SERIES, lw=2.0, ms=5.5,
                mec=SERIES, mfc=SERIES, zorder=3)
    ...
    if probes:
        ...
        ax.plot(xs, ys, "o", color=SERIES, ms=5.5, mew=1.6,
                mfc=PROBE_FACE, zorder=3)
        for x, y, name in probes:
            ax.annotate(name, xy=(x, y), xytext=(0, 7),
                        textcoords="offset points", ha="center",
    ...
    ys_all = ([y for _, y, _ in ladder] + [y for _, y, _ in probes]
              + [100 * s["top1"] for s in anchors.values()] + [ceiling])
