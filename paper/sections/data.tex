...
kind. ``Unseen'' describes the set, not every card in it: BRO/TMT/SOS
...
confers no advantage under the zero-shot contract above -- a reprint has no
memorized identity to exploit, only the same frozen feature vector any
novel card with identical features would get -- but it does mean set-level
and card-level novelty are not the same claim.
