...
\paragraph{The post-day-one ceiling.}
The protocol pre-registers one follow-up model, executed only after the
zero-shot pass and never iterated: a per-set ceiling trained directly on
...
recovers \NormScoreMsh\% of the ceiling by the pre-registered normalized
...
itself. One caveat belongs next to that: the MSH ceiling saw only the
frozen early snapshot, so it is a ceiling under day-one-era data volume,
not the stronger ceiling a full season of picks would train.

\paragraph{Disclosure: a deviation in the quarantine.}
While running that post-day-one step we found, and report here, one
deviation from the quarantine as written. The deployed draft assistant
that this research grew out of retrains its own per-set serving models
on a nightly schedule, and that standing job downloaded the public MSH
file and trained the assistant's MSH serving model roughly 21 hours
before the frozen evaluation executed. The protocol sentence ``no
training may read MSH pick data until the frozen evaluation has run''
was therefore violated by an unattended job outside the paper's
pipeline. The evaluation itself is unaffected, and verifiably so: every
battery member's weights were hash-anchored in commits that predate not
only the evaluation but the file's first download, the file the cron job
fetched is byte-identical to the frozen snapshot (same sha256 and ETag),
and no battery artifact or metric implementation changed after the
freeze. The serving model it trained shares no parameters with any
battery member; it is, in fact, the same recipe as the pre-registered
ceiling above, and retraining that recipe cleanly after the evaluation
reproduces it exactly, which is how the deviation was noticed. We report
it because the protocol's credibility rests on disclosing exactly this
kind of thing.
...
them pending rather than backfill them
(\pending{post-release baselines: recipes were never frozen pre-T0}).
