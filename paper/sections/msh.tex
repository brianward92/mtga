...
The substantive protocol was tagged as \texttt{eval-protocol-v1} on July
4, before the first public \seventeenlands{} MSH file existed. That file
was downloaded automatically at 02:30 EDT on July 7 and immediately
identified by sha256 and ETag. The enforcement-hardening tag
\texttt{\ProtocolTag}, which contains the exact battery manifest, was cut
at 20:20 EDT that day; the frozen snapshot fields were committed at 21:25,
and the single frozen pass ran at 23:59. Each battery member's weight hash
was already committed before the 02:30 download. Running a second battery
pass would require a new public tag and disclosure that a second round
happened.
...
before embedding, \S\ref{sec:features}). The dev-trio ablations show no
transfer failure on the sole licensed development set, but do not isolate
the effect of masking (\S\ref{sec:analysis}). The BRO development row provides the
same-holdout anchor against that prior. These results show that scale over
public card features can be competitive with published systems that use
post-release data; because the headline and prior use different test sets,
they do not establish a controlled superiority ranking. This last sentence
narrows the pre-written Band C interpretation after audit; the selected
band and every reported number are unchanged.
% ======================================================================
...
(\S\ref{sec:analysis}), is \FfullMshEce{}, below F-dev's calibrated
dev-trio mean of \DevMeanEceAtFrozenT{} at the same temperature. The
comparison crosses from tower-bearing F-dev to tower-free F-full, so we do
not interpret it as an architecture ranking. The low MSH ECE is not simply forced picks
...
\paragraph{The post-day-one supervised reference.}
The protocol pre-registers one follow-up model, executed only after the
zero-shot pass and never iterated: a within-set reference trained directly on
the frozen MSH snapshot's own picks with the stock per-set recipe. It
reaches \CeilMsh\% on its held-out validation drafts
(Table~\ref{tab:main}). Scored on those identical held-out picks, F-full
recovers \NormScoreMsh\% of the reference by the pre-registered normalized
score, and \LateRetMsh\% restricting to picks eight and later of each
pack, over \MshSharedPicks{} shared picks. Both figures are higher than
their dev-trio counterparts: on the one set where nothing was tuned, the
zero-shot model comes closest to a model trained on the target set
itself. One caveat belongs next to that: the MSH reference saw only the
frozen early snapshot, so it is weaker than a target-trained model using a
full season of picks might be.

\paragraph{Disclosure: a deviation in the quarantine.}
The deployed draft assistant that this research grew out of also had a
standing nightly job. That separate pipeline downloaded the public MSH
file and trained its own per-set serving model roughly 21 hours before the
frozen evaluation, violating the protocol's broad statement that no
training could read MSH before the pass. The core zero-shot evaluation is
separable and auditable: all frozen-battery weight hashes were committed
before the first download; the serving model shares no parameters with any
battery member; the snapshot bytes have the same sha256 and ETag; and no
battery artifact changed after \texttt{\ProtocolTag}. A clean post-pass
training run reproduced the serving model's metrics, which is how the
deviation was found. We report it because it limits the literal quarantine
claim even though it does not change the frozen battery's predictions.

\paragraph{What remains open.}
The second pre-registered post-day-one row, F-full fine-tuned on MSH's
own picks, has not been run. The three asterisked post-release baselines
of Table~\ref{tab:baselines} also remain open for a different and more
instructive reason: the protocol named them, but their exact recipes
were never frozen in the battery before the snapshot arrived, and
defining them after seeing the data would make them post-hoc. We leave
them unevaluated rather than define them after the fact.
