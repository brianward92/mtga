...
zero-shot drafting problem and remain its only published quantitative
result. They replace one-hot card identities with concatenated feature
vectors---hand-engineered attributes plus a Sentence-BERT
...
from \seventeenlands{}---inside a contextual-preference-ranking model
trained with a triplet loss. Two of their findings shape our design: feature
...
single-set training; no multi-set features-only number is published. Filling
that gap---multi-set training, public card information only---is this
paper's headline experiment, and we adopt their held-out BRO as a
...
picks. Zero-shot GPT-4o reaches 43\%---below the hour-zero heuristics we
measure (\S\ref{sec:results})---and providing full rules text in the prompt
lowers it; open 7--8B models fail to pick legal cards until LoRA-tuned on a
million picks from the target set, after which they still trail small
supervised MLPs at orders of magnitude more inference cost. We take the
implied guidance: use a frozen text \emph{embedding} as one feature block,
not a language model as the policy.
...
unseen cards---the closest cross-game precedent for text-mediated transfer,
applied to gameplay rather than drafting.
