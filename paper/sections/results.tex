...
\begin{table}[t]
  ...
  \makebox[\textwidth][c]{\input{tables/main_results}}
\end{table}
...
sets, the within-set supervised reference, reaches \CeilBro\% /
\CeilTmt\% / \CeilSos\%.
DraftFM's pre-registered normalized score, its accuracy as a fraction of
that reference on identical held-out picks per \S\ref{sec:protocol}, is
...
the size of the reference model's own held-out split. Excluding forced
...
is below that number, but the two use different information regimes: their
number assumes information that does not exist on release day. The same
paper's 33.6--35.6\% features-only result, under single-set training, is a
relevant release-day reference rather than a controlled comparison. Against that
...
Figure~\ref{fig:tmt-tierlist} shows the whole-set P1P1 ranking for a TMT
card. For sets that are in the training corpus but have no dedicated
...
(\S\ref{sec:analysis}). Whole-set P1P1 expected value \VBroEv, the
\VBroPercentile{}th percentile of BRO's card pool by the deployed per-set
model's own ranking. Two selected comparison cards,
\VBroRunnerOneName{} and \VBroRunnerTwoName{}, rank at the
\VBroRunnerOnePercentile{}th and \VBroRunnerTwoPercentile{}th. These are
whole-set percentiles, not candidates recovered from a logged pack.}
...
described in \mtg{} rules-text style. Whole-set P1P1 expected value \VTmtEv,
...
\begin{table}[t]
  ...
  players, deployment mode. Rows marked $^{*}$ were named in the protocol
  but not evaluated because their exact recipes were not frozen before
  the snapshot. Published numbers from prior work, which differ in test
...
post-release baseline rows remain unevaluated; their recipes were never
frozen before the snapshot arrived, and \S\ref{sec:msh} explains why we
do not define them after the fact.
...
\begin{figure}[t]
  ...
  mark published anchors and the within-set supervised-reference mean.}
  \label{fig:scaling}
\end{figure}
