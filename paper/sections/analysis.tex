...
\begin{table}[t]
  ...
  architecture variants tested against the Full baseline before the
  deployed architecture was fixed. A-notext and A-noUB are the two
  pre-registered comparisons (\S\ref{sec:protocol}) analysed below:
  A-notext drops the 384-d text embedding from the Full recipe, and A-noUB
...
model on the day it matters, which is why the protocol makes a licensed-IP
set (TMT) a development set: the rehearsal happens where it can still
inform design. One mitigation is built in: card names are masked out of
the text before embedding (\S\ref{sec:features}), so the encoder sees
mechanics, not franchises. A second design choice does not reduce the
shift but makes it measurable: the comparison grid of
Table~\ref{tab:ablations} separates what the text embedding contributes on
in-universe sets (SOS, BRO) from what it contributes on licensed sets
(TMT, MSH).

Zero-shot, the licensed dev set turns out not to be the hard one. TMT
reaches \FdevTmt\% against SOS's \FdevSos\% (and \RatioTmt\% vs
\RatioSos\% of their respective ceilings), which suggests masking plus
structured features are carrying most of the load. That could mean text
simply adds little on licensed sets, or it could just mean TMT is an
easier format. The grid distinguishes between the two:
...
the licensed-IP set, which favors the masking-plus-structured-features
explanation over TMT simply being an easier format. The UB-penalty
(removing the three licensed-IP training sets from A-noctx) tells a
consistent story: \UbPenBro pp on BRO (CI \UbPenBroCi pp), \UbPenTmt pp on
TMT (CI \UbPenTmtCi pp), \UbPenSos pp on SOS (CI \UbPenSosCi pp). Losing
...
\RatioBro\% of its ceiling. Of three candidate explanations, the bonus
...
quality does. MSH shows the same pattern: deployment mode (conditioning on
a fixed top skill bucket) and human mode (conditioning on each drafter's
own bucket) differ by only \FfullMshSkillGap{}pp on the expert slice, so
the choice of which bucket to condition on barely matters there either.
...
architectures this way. MSH ECE, at this same frozen temperature, is
\FfullMshEce, tighter than any of the three dev-trio numbers above rather
than in between them. We do not have a confirmed explanation for that on
this evidence -- it is not simply forced picks inflating the count of
trivially well-calibrated predictions, since MSH's non-forced ECE is close
to its forced-inclusive figure -- so we report it without
...
against the per-cell random floor show this directly. Excluding forced
picks, zero-shot top-1 never comes closer than 20.5 (BRO) / 32.5 (TMT) /
28.4 (SOS) percentage points to chance at any point in the draft,
including its latest picks.
