...
and every table cell and prose number is emitted by a script from that
ledger and cached evaluation outputs -- no result number here is typed
by hand. The ledger schema, frozen identifiers (protocol tag, manifest
hashes), release contents, and script-level pointers for reproducing any
specific number are in the companion implementation reference, not
repeated here.
...
Figures~\ref{fig:teaser}, \ref{fig:bro-tierlist}, \ref{fig:closecall},
\ref{fig:tmt-tierlist}, and \ref{fig:gallery} reproduce official Scryfall
...
of the Coast produced or endorses this work. The cards illustrated are
drawn from BRO, SOS, TMT, LTR, LCI, and DMU -- all public, released
sets (\S\ref{sec:data}) -- never from MSH, which remains untouched until
the frozen evaluation of \S\ref{sec:protocol}.
